						...
						if (flag == 1) { // 数据报还没有传完
							...
							try {
								...
								if (waitCount > 20) { // 超过20次等待ack 则判定为超时，重传
									int list[] = new int[3];
									list = server.timeoutHandler(curSeq, curAck, ack, totalSeq);
									curSeq = list[0];
									curAck = list[1];
									totalSeq = list[2];
									waitCount = 0;
//									System.out.println(curSeq+"\\"+curAck);
							...
							if (flag == 1) {
								...
								curAck = server.ackHandler(dataFromClient[0], curAck, ack);
...
//								System.out.println("=======================");
				...
				while (true) { // 建立握手并不断接收服务器的数据包
					...
					if (stage == 1) { // 等待接收数据阶段
						seq = (int) dataFromClient[0];
						if (seq == 0) {
							break;
						}
						byte[] dataToClientAck = new byte[BUFFER_LENGTH];
						...
						if ((waitSeq - seq) == 0) {
							...
							String data = new String(dataFromClient, 1, dataFromClient.length - 1);
							System.out.println("接收到的数据为: " + data); // 输出数据

							recvSeq = seq;
							dataToClientAck[0] = (byte) seq; // 生成确认报文序列号的ACK报文
							dataToClientAck[1] = '\0';
						}
						// 发送客户端回复报文（ack）
						DatagramPacket packetToClientAck = new DatagramPacket(dataToClientAck, dataToClientAck.length,
								clientAddress, clientPort);
						server.serverSocket.send(packetToClientAck);
						System.out.println("send a ack of " + (int) dataToClientAck[0]);
					} // stage
					Thread.sleep(500);
				} // while true stage 建立握手并不断接收服务器的数据包
			}
			if (!cmd.equals("-testgbn") && !cmd.equals("-returntrans")) {
